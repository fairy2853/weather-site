...
button.addEventListener("click",function(){
  ...
  getCoords(`https://api.openweathermap.org/data/2.5/forecast?q=${document.getElementById('search').value}&lang=ua&appid=1ce9370a31ca136fdf72c09c9c571ecf`)
})
...
function getCoords(url){
  navigator.geolocation.getCurrentPosition((position) => {
    ...
    arr.forEach(e=>{
            ...
            info.innerHTML=
            `
            <div class="curr_temp"><div><p>${Math.floor(e.list[0].main.temp-273)}&deg</p></div></div>
            <div class="icon">
      ...
      e.list.forEach(r=>{
        ...
        if(Number(dt.getHours())>0 && Number(dt.getHours())<=9){
          ...
          <p>${Math.floor(r.main.temp-273)}</p>
          <p>${Math.floor(r.main.feels_like-273)}</p>
          <p>${r.main.humidity}</p>
